...
class SAECacheLayer(CacheModule):
    ...
    def __init__(
        ...
        self.decoder = CacheLayer.from_dims(
            d_in=cfg.d_dict, d_out=cfg.d_data, bias=False
        )
        resampler = resampler or resampler_factory(
            cfg.resampler_cfg, W_next=self.decoder.W
        )
        resampler.W_next = self.decoder.W
        ...
        if cfg.tied_init:
            self.decoder.W.data.transpose(-2, -1)[:] = self.encoder.cachelayer.W.data
        ...
        cache.y_pred = (
            y_pred := (
                self.decoder(
                    ...
                    cache=cache["decoder"].add_cache_ignore("acts"),
...
class SAETrainer:
    ...
    def norm_dec_grads(self):
        grad = self.sae.cachelayer.decoder.W.grad
        dec_normed = (
            self.sae.cachelayer.decoder.W.data
            / self.sae.cachelayer.decoder.W.data.norm(dim=0, keepdim=True)
        )
        grad_orth = grad - (dec_normed * grad).sum(0, keepdim=True) * dec_normed
        self.sae.cachelayer.decoder.W.grad[:] = grad_orth

    def norm_dec(self):
        if self.t % 1000 == 0:
            print("norm dec", self.sae.cachelayer.decoder.W.shape)
        self.sae.cachelayer.decoder.W.data[:] = (
            self.sae.cachelayer.decoder.W.data
            / self.sae.cachelayer.decoder.W.data.norm(dim=0, keepdim=True)
        )
